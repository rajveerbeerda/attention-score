...
from facial_expressions_recognizer import *
from detect_voice import *
...
audio_dir = os.path.join('instance', 'audios')
frame_dir = os.path.join('instance', 'frames')
video_dir = os.path.join('instance', 'videos')
...
def main_function():
    ...
    f = open("instance/time-for-videos.txt", "r")
    s = str(f.read()).split(",")
    f.close()
    ...
    f = open("instance/time-for-videos.txt", "w")
    if end_time>min_video_duration:
        f.write(str(start_time) + "," + str(end_time))
        f.close()
        return {}, 0
    else:
        f.write(str(end_time + 10) + "," + str(end_time + 15))
    f.close()
